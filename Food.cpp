...
Food::Food() {
  ...
  string names[tierCount][nameCount] = {// Snack
                                        {"Berries", "Nuts", "Crackers"},
                                        // Meal
                                        {"Beans", "Bread", "Porridge"},
                                        // Feast
                                        {"Hog", "Venison", "Steak"}};

  // The rating will determine which adjective
  string adjectives[ratingCount] = {"Stale", "Fresh", "Vital"};
...
// pay for the food
int Food::getCost() { return cost; }

// Restore energy
int Food::getStrength() { return energy; }

ObstacleType Food::getObstacle() { return BOULDER; }

int Food::getDetails(string &source, int &y, int &x) { return 0; }

int Food::getMoney() { return 0; }

int Food::getTruth() { return 0; }

void Food::setClue(string clue, int y, int x) { return; }
