      ...
      // find one of the other 7 kinds of items
        ...
        while (!temp || !isValid) {
          ...
          if (temp) {
            ...
            isObstacle = (temp->itemType == NULL ||
                          temp->itemType->getType() == OBSTACLES);
  ...
  if (player.hasClue(y, x)) {
    ...
    if (itemType->getDetails(clue, targetY, targetX)) {
      ...
      if (!itemType->getTruth() &&
          (abs(py - targetY) <= dist && abs(px - targetX) <= dist)) {
        clue = "FOOL! There is no diamond here. You have fallen victim to one "
        ...
        if (abs(targetY - py) == 0 && abs(targetX - px) == 0) {
          ...
          clue = "You are " + std::to_string(px) +
                 " grovnicks from the western border.               ";

          if (!temp->itemType)
            clue += "There " + plural + " " + article + " " + typeName +
                    " that " + plural + " " +
                    player.itemDirect(truth, targetY, targetX);
          else
            clue += "There " + plural + " " + article + " " + typeName +
                    " that " + plural + " " +
                    player.itemDirect(truth, targetY, targetX);
...
bool Engine::isGameWon() { return gameWon; }
...
void Engine::displayWin() {
  ...
  char ending_message2[] =
      "You found the Royal Diamond worth one zillion zillion whiffles!!!";
  ...
  counter =
      35; // counter for loops, reset for horizontal lines of rectangle creation
  ...
  mvprintw(
      y - 1, x - 6, "%s",
      ending_message); // prints hello world! message in center of rectangle
  mvprintw(
      y, x - 32, "%s",
      ending_message2); // prints hello world! message in center of rectangle
  mvprintw(
      y + 10, x + 14, "%s",
      ending_message3); // prints hello world! message in center of rectangle
...
void Engine::displayLose() {
  ...
  counter =
      14; // counter for loops, reset for horizontal lines of rectangle creation
